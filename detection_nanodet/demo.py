...
lock = threading.Lock()
array = np.empty((20, 10), np.chararray)
...
def parse_timeline():
    ...
    while True:
        # ast = parse.parse(grammar, "Person picks cup")
        # pprint.pprint(ast)
        lock.acquire()
        # Flatten array to create string to parse
        flattenedMatrix = array.flatten()
        lock.release()
        # newFlattened = np.array([], np.chararray)
        str = ''
        for item in flattenedMatrix:
            if item is not None:
                # newFlattened = np.append(newFlattened,item)
                str = str + item + ' '
        str = str[:-1]
        # pprint.pprint(str)
...
def vis(preds, res_img, letterbox_scale, fps=None):
    ...
    for pred in preds:
        if pred[-1].astype(np.int32) == 0 or pred[-1].astype(np.int32) == 39:
            bbox = pred[:4]
            conf = pred[-2]
...
if __name__ == '__main__':
    ...
    if args.input is not None:
        ...
        while cv.waitKey(1) < 0:
            ...
            lock.acquire()
            array[frameNumber] = arrayClasses
            lock.release()
